            ...
            try {
                const string = `SELECT
                                ...
                                COUNT(if(r.reaction = 'dislike', 1, NULL)) AS dislikes,
                                    (SELECT reaction FROM Likes WHERE Users_id = ? AND  Posts_id = r.Posts_id) AS userReaction
                                FROM posts AS p
                                LEFT JOIN Likes AS r ON p.id = r.Posts_id
...
// POST Likes to posts Controller
//==========================================================================================================
    ...
    switch (reaction) {
        case "like": // Like Post
            try {
                let string;
                if (!reacted) {
                    string = "INSERT INTO Likes (Posts_id, Users_id, reaction) VALUES (?, ?, 'like')";
                } else {
                    string = "UPDATE Likes SET reaction = 'like' WHERE Posts_id = ? AND Users_id = ?";
                }
        ...
        case "dislike": // Dislike Post
            try {
                let string;
                if (!reacted) {
                    string = "INSERT INTO Likes (Posts_id, Users_id, reaction) VALUES (?, ?, 'dislike')";
                } else {
                    string = "UPDATE Likes SET reaction = 'dislike' WHERE Posts_id = ? AND Users_id = ?";
                }
                const inserts = [post_id, user.id];
                const sql = mysql.format(string, inserts);
        ...
        case "null": // Like ou Dislike Post
            try {
                const string = "UPDATE Likes SET reaction = 'null' WHERE Posts_id = ? AND Users_id = ?";
                // const string = "DELETE FROM Likes WHERE Posts_id = ? and Users_id = ?";
                const inserts = [post_id, user.id];
                const sql = mysql.format(string, inserts);
